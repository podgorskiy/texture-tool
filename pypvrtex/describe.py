...
def describe(self):
    assert isinstance(self, pypvrtex.PVRTexture)
    s = '<' + '\n'
    members = [attr for attr in dir(self) if not callable(getattr(self, attr)) and not attr.startswith("__")]
    for attr in members:
        s += '\t' + attr + ': ' + str(getattr(self, attr)) + '\n'
    s += '\t' + str('Flipped X: ' + str(self.get_orientation(pypvrtex.Axis.x))) + '\n'
    s += '\t' + str('Flipped Y: ' + str(self.get_orientation(pypvrtex.Axis.y))) + '\n'
